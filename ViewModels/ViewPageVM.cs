...
namespace LabelingMonitor.ViewModels
{
    class ViewPageVM : BindableBase
    {
        ...
        { 
            get
            {
                if (MarkerType == mainWindowVM.MARKER_TYPE_FRAME)
        ...
        private void InitializeVariables()
        {
            ...
            MarkerType = mainWindowVM.MarkerType;
        ...
        {
            if (e.PropertyName == nameof(mainWindowVM.MarkerType))
            {
                MarkerType = mainWindowVM.MarkerType;
                ResetViews();
                UpdateImageInfo();
            }
            if (e.PropertyName == nameof(mainWindowVM.NeedUpdate))
            {
                if(mainWindowVM.NeedUpdate)
                {
                    UpdateImageInfo();
                    mainWindowVM.NeedUpdate = false;
                }
            }
        }
        ...
        public void UpdateImageInfo()
        {
            try
            {
                ...
                if (UserData.GetCountOfParcedImages(MarkerType) > 0)
                {
                    ...
                    if (mainWindowVM.MarkerType.Equals(mainWindowVM.MARKER_TYPE_FRAME))
                    {
        ...
        {
            if (mainWindowVM.MarkerType == mainWindowVM.MARKER_TYPE_FRAME)
            {
                CmbEnabled = false;
